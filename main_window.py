...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QDialog, QVBoxLayout, QHBoxLayout, QLabel,
                             QLineEdit, QPushButton, QComboBox, QCheckBox, QColorDialog, QFileDialog, QMessageBox)
...
class SubWindow(QDialog):
    ...
    # 定义一个信号，用于传递参数
    closed = pyqtSignal(tuple, str, str, str, str, bool)  # [figsize, legend1_loc, legend2_loc, title_fontweight, color_path, show_lack]
    ...
    def __init__(self, current_args):
        ...
        self.show_lack = QCheckBox("显示缺失时间段")
        ...
        self.show_lack.setChecked(current_args[5])
        ...
        row3_layout.addWidget(self.show_lack)

        row4_layout = QHBoxLayout()
        row4_layout.addWidget(self.save_set)
    ...
    def save_and_close(self):
        ...
        show_lack = self.show_lack.isChecked()

        # 发射信号，传递参数
        self.closed.emit((figsize_x, figsize_y), legend1_loc, legend2_loc, title_fontweight, color_path, show_lack)
...
class MainWindow(QMainWindow):
    ...
    def open_sub_window(self):
        # 创建副界面实例并显示
        self.sub_window = SubWindow([self.figsize,self.legend1_loc,self.legend2_loc,self.title_fontweight,self.color_path,self.show_lack])
        self.sub_window.closed.connect(self.handle_subwindow_closed) # 连接 SubWindow 的信号
        self.sub_window.exec_()  # 使用 exec_() 显示模态对话框

    def handle_subwindow_closed(self, figsize, legend1_loc, legend2_loc, title_fontweight, color_path, show_lack):
            ...
            if file_path:
                try:
                    ...
                    drawer = cash_flow_diagram.Drawer(rd=file_path, sheet_name=sheet_name, title=title,
                                                      ...
                                                      colors=colors, show_lack=self.show_lack)
                    drawer.draw()
                except Exception as e:
